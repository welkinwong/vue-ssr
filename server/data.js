...
export default function patchSubscribeData(VueSSR) {
  ...
  Mongo.Collection.prototype.find = function(selector = {}, options = {}) {
    ...
    if (ssrContext && !VueSSR.inSubscription.get()) {
      ...
      // 返回带有 limit 的 cursor 会导致服务端控制台报以下错误
      // error caught on publication:  *** :  Must use an ordered observe with skip or limit (i.e. 'addedBefore' for observeChanges or 'addedAt' for observe, instead of 'added').
      // 该网址解释为 minimongo 的问题 https://github.com/meteor/meteor/issues/1643
      // 删除掉 limit 后正常，暂时未发现影响
